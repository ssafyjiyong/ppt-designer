...
**Opus 4.6은 글로벌 단일 추론 프로파일(`global.anthropic.claude-opus-4-6-v1`)을 사용합니다.** Sonnet 4.6과 마찬가지로 APAC/US/EU 리전별로 나뉘지 않습니다.
...
aws bedrock list-inference-profiles --region ap-northeast-2 \
  --query "inferenceProfileSummaries[?contains(inferenceProfileId, 'opus-4-6')].[inferenceProfileId,status]" \
  --output table
```
...
> 🚨 모델 ID(`anthropic.claude-opus-4-6-v1`)를 그대로 쓰면 호출 시 `on-demand throughput isn't supported` 에러가 납니다. 반드시 `global.` 접두사가 붙은 추론 프로파일 ID를 써야 합니다.
>
> 💡 위 명령 결과의 프로파일 ID가 `global.anthropic.claude-opus-4-6` (끝의 `-v1` 없음) 으로 나온다면 그 값을 그대로 쓰세요. AWS는 동일 모델군 안에서도 버전 suffix 표기가 다를 수 있습니다.
>
> 💡 다른 모델로 바꿀 때는 위 `list-inference-profiles` 명령으로 prefix를 먼저 확인하세요. 신규 모델(Opus 4.6, Sonnet 4.6)은 `global.`, 구형 모델은 `apac./us./eu.` 등 prefix가 다릅니다.
...
Parameter BedrockModelId [global.anthropic.claude-opus-4-6-v1]: [Enter]
...
| `ValidationException ... on-demand throughput isn't supported` 또는 `model identifier is invalid` | 추론 프로파일 ID가 잘못됨. 모델 ID(`anthropic.claude-...`)를 그대로 썼거나, Bedrock 콘솔의 실제 프로파일 ID와 철자가 다른 경우. CloudFormation 콘솔 → 스택 → **Update** → **Use existing template** → Parameters에서 `BedrockModelId`를 콘솔의 실제 ID(`global.anthropic.claude-opus-4-6-v1` 등)로 수정 |
